...
typedef int (*orig_getc_type)(FILE *stream);

int getc(FILE *stream){
    char* fail_num = getenv("GETC_FAIL");
    ...
    if (fail_num != NULL && original == 0){
        ...
        for (i = 0; i < length; i++) {
            if (numbers[i] == global_counts.getc) {
                error = -1;
                break;
            }
        }
    ...
    orig_getc_type orig_getc;
    orig_getc = (orig_getc_type)dlsym(RTLD_NEXT, "getc");
    return orig_getc(stream);
}